...
public class ControlJet : MonoBehaviour
{
    ...
    {  
        ...
        gameObject.transform.Rotate(Vector3.up, -Mathf.Rad2Deg * terrain.GetPlayerStartAngle()); //orient the player correctly
    ...
    void FixedUpdate()
    {
        ...
        Aef.delay = Mathf.Max(10, transform.position.y - terrain.GetHeightOfTerrain(gameObject.transform.position));
    }

    private void GroundControls()
    {
        float runwayHeight = terrain.GetHeightOfRunway(gameObject.transform.position.x, gameObject.transform.position.z);
    ...
    private void AirControls()
    {
        ...
        if(flightState == FlightState.Stall)
        {
            ...
            attackAngleChange = (1 - Vector3.Dot(gameObject.transform.forward, Vector3.down)) * Time.fixedDeltaTime * 20;
       ...
       // }
        ...
        gameObject.transform.Rotate(Vector3.right, attackAngleChange);     
...
*/
    ...
    public void Crashed()
    {
        ...
        GameObject explosion = Instantiate(destructionEffects[0], gameObject.transform.position, gameObject.transform.rotation);
        ...
        if(destructionEffects[1] != null)
        {
            ...
            float groundHeight = terrain.GetHeightOfTerrain(gameObject.transform.position);
           
            GameObject crashDecal = Instantiate(destructionEffects[1], new Vector3(gameObject.transform.position.x, groundHeight+0.05f, gameObject.transform.position.z), Quaternion.Euler(90, 0, 0));
           
            crashDecal.transform.forward = terrain.GetTerrainNormal(gameObject.transform.position) * -1f;
